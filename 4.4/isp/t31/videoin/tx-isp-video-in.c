		...
		const unsigned short *probe_addrs)
{
	...
	struct device_driver *drv = client ? client->dev.driver : NULL;
	if (client == NULL || drv == NULL)
		goto error;

	/* Lock the module so we can safely get the tx_isp_subdev pointer */
	if (!private_try_module_get(drv->owner))
		goto error;
	sd = private_i2c_get_clientdata(client);

	/* Decrease the module use count to match the first try_module_get. */
	private_module_put(drv->owner);
...
static long subdev_sensor_ops_register_sensor(struct tx_isp_subdev *sd, void *arg)
{
	...
	if(info->cbus_type == TX_SENSOR_CONTROL_INTERFACE_I2C){
		...
		ISP_WARNING("%s[%d] the type of sensor SBUS hasn't been defined.\n",__func__,__LINE__);
		return -EINVAL;
	}
...
static long subdev_sensor_ops_release_sensor(struct tx_isp_subdev *sd, void *arg)
{
	...
	if(sensor == vi->active){
		private_mutex_unlock(&vi->mlock);
		ISP_WARNING("the sensor is active, please stop it firstly.\n");
		return -EINVAL;
	}
...
static long subdev_sensor_ops_set_input(struct tx_isp_subdev *sd, int *index)
{
	...
	if(sensor){
		if(vi->state == TX_ISP_MODULE_RUNNING){
			ISP_WARNING("Please, streamoff sensor firstly!\n");
			return -EPERM;
		}
		...
		if(ret != ISP_SUCCESS){
			ISP_WARNING("Failed to deinit the pipeline of %s.\n", sensor->attr.name);
			goto err_exit;
		}
	...
	ret = tx_isp_call_subdev_notify(sd, TX_ISP_EVENT_SUBDEV_INIT, &enable);
	if(ret != ISP_SUCCESS){
		ISP_WARNING("Failed to init the pipeline of %s.\n", sensor->attr.name);
		goto err_init;
	}
...
static int tx_isp_vin_init(struct tx_isp_subdev *sd, int on)
{
	...
	if(sensor){
		...
		ISP_WARNING("[%d] Don't have active sensor!\n",__LINE__);
		ret = -EPERM;
	}
...
static ssize_t video_input_cmd_set(struct file *file, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	if(IS_ERR_OR_NULL(sensor)){
		seq_printf(m ,"don't have active sensor, please set sensor firstly!\n");
		return len;
	}
